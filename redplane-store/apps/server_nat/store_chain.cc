...
void req_handler(std::string NextIpAddr)
{
    // Key-value store
    ...
    redplane::RawTransport *raw_transport = new redplane::RawTransport(kChainUDPPort, kPhyPorts, send_buf, redplane::RawTransport::kSendRingSize);
    ...
    while (true)
    {
        ...
        if (num_pkts > 0)
        {
            ...
            for (size_t i = last_rx_ring_idx; i < last_rx_ring_idx + num_pkts; i++)
            {
                ...
                state_store[req_pkt->req_header.flow_key] = req_pkt->req_header.value;
...
int main(int argc, char **argv)
{
    ...
    {
        req_handler_thread[i] = std::thread(req_handler, FLAGS_nextip);
